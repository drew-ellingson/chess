...
class GameState:
    ...
    ks_castle_rights: Dict[str, bool] = field(default_factory=lambda: {'W':True, 'B':True})
    qs_castle_rights: Dict[str, bool] = field(default_factory=lambda: {'W':True, 'B':True})
    ...
    def generate_valid_moves(self):
        ...
        for i, row in self.board:
            for j, pc in row:
                ...
                if self.get_piece(i, j).startswith(color):
                    valid_moves.extend(self.gen_valid_pc_moves(self, i, j))
        return valid_moves
...
class Move:
    ...
    def has_illegal_collisions(self):
        if self.gs.get_piece(*self.sq_1)[0] == self.color: # can't land on same color piece
            return True
        ...
        intermediate_sqs = [_add(self.sq_0, _mult(i, diff_dir)) for i in range(1, max_diff)]
